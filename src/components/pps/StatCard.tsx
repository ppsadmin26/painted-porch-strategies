...
export default function StatCard({
  ...
  if (variant === "editorial") {
    return (
      <div className={cn("border-l-4 border-gold pl-5 py-2", className)}>
        <p data-body-allow className={cn("text-[1.375rem] md:text-pullquote font-poppins font-bold leading-tight !not-italic", accentClass)}>
          {s.figure}
  ...
  return (
    ...
    >
      ...
      {framing && (
        <p className="text-body mt-4 -sm text-foreground/80 leading-relaxed italic">{framing}</p>
      )}
