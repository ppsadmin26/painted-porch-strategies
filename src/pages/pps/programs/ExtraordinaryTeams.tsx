...
const faqs = [
  ...
  { question: "Where are the Private Communities located?", answer: "Our communities are hosted on a private platform called Mighty Networks, a completely private network away from data scraping and advertisers, consisting of only our members. It's free to join and easy to use." },
              ...
              <AnimatedSection key={tier.name}>
                <div className={`rounded-2xl border-2 p-8 relative ${tier.popular ? "border-pps-teal shadow-lg" : "border-border"}`}>
                  ...
                  <Button asChild className={`w-full font-poppins font-semibold rounded-lg ${tier.popular ? "bg-pps-teal hover:bg-pps-teal/90 text-white" : "bg-pps-navy hover:bg-pps-navy/90 text-white"}`}>
                    <Link to="/contact?scope=Yourself&interest=self-paced&message=I'm interested in the Extraordinary Teams program.">{tier.cta}</Link>
                  </Button>
                </div>
              </AnimatedSection>
            ))}
