  ...
  const load = async () => {
    ...
    const [offRes, launchRes] = await Promise.all([
      supabase
        ...
        .select("id, offering_key, name, facilitator, tier, engagement_tier, delivery_format, blurb, description, current_url, dedicated_url, anchor_id, is_live, is_published, sort_order, topic, topic_slug, include_in_workshops, is_featured_in_quiz, is_keynote, include_on_speaker_page, image_url, launch_slug, b2c_rt_pools, b2b_rt_pools, blue_door_required, workshop_card_challenge, workshop_card_description, workshop_card_format, workshop_card_investment, workshop_card_bullets, updated_at")
        .order("name"),
