...
const WATCH_PATH = "/6-communicator-styles-watch";
const TEMPLATE_NAME = "communicator-styles-replay";
const SOURCE_LABEL = "6 Communicator Styles Replay";
const TAGS = ["6Comm Replay", "newsletter"];
      ...
      try {
        const emailRes = await supabase.functions.invoke("send-transactional-email", {
          body: {
            ...
            idempotencyKey: `comm-styles-replay-${magicToken}`,
            templateData: { firstName, watchUrl: magicLink },
          },
        });
        if (emailRes.error) {
          console.error("Comm Styles replay email send failed:", emailRes.error);
        }
      } catch (sendErr) {
        console.error("Comm Styles replay email exception:", sendErr);
      }
    }
